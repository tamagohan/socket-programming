...
fn main() {
    ...
    .into_iter().find(|iface| iface.name == *interface_name).expect("Failed to get interface");

    let (_ts, mut rx) = match datalink::channel(&interface, Default::default()) {
